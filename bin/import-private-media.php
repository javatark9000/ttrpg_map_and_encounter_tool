...
try {
    $driverId = (int) $db
        ...
        ->fetchColumn();
    $visibilityId = (int) $db
        ->query("SELECT id FROM visibility_levels WHERE code='dm_only'")
        ->fetchColumn();
    $purposes = $db
        ->query("SELECT code,id FROM media_purposes WHERE code IN ('token','portrait')")
    ...
    $findAsset = $db->prepare(
        'SELECT id FROM media_assets WHERE storage_path=? ORDER BY id LIMIT 1',
    );
    ...
    foreach ($matches as $match) {
        ...
        if (
            $absolutePath === false ||
            !str_starts_with($absolutePath, $uploads) ||
    ...
    $db->commit();
    printf(
        "Media enlazado: %d; criaturas no encontradas: %d; archivos no encontrados: %d\n",
        $linked,
        count($missingCreatures),
        count($missingFiles),
    );
    if ($missingCreatures) {
        fwrite(
            STDERR,
            'Sin criatura: ' . implode(', ', array_slice($missingCreatures, 0, 20)) . "\n",
        );
    }
    if ($missingFiles) {
